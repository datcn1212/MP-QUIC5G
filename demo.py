...
CLIENT_CMD2 = "/App/quic/client_mt -m https://10.0.0.20:6121/demo  > /App/logs/client2.logs 2>&1"
...
def exec_test(server_cmd, rtt, tcp_traffic):
    ...
    start = time.time()
    client2.sendCmd(CLIENT_CMD2)
    
    # Timeout of 20 seconds for detecting crashing tests
    output = client2.monitor(timeoutms=20000)

    # Check for timeout
    if client2.waiting :
        delta = 20
        client2.sendInt()
        client2.waiting = False
        network.stop()
        time.sleep(1)
        network.cleanup()
    else:
